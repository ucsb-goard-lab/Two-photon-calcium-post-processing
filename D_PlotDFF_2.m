function [] = D_PlotDFF_2(data,fig,currPage,stopFlag)
...
for i = 1:numTraces
	...
	try plot(xAxis,data.DFF2((currPage-1)*numTraces+i,:))
		yAxisVal = ylim;
	...
	catch plot(zeros(1,size(data.DFF2,2)));
		xlim([xAxis(1) xAxis(end)]);
	end
end
...
%%% callback functions
	function buttonCallback(src,evt)
		...
		disp('Finished')
		stopFlag = 1;
		D_PlotDFF_2(data,fig,currPage,stopFlag);
